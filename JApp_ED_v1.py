...
beam_thickness_val = 1.3
beam_modulus_val = 210000
beam_width_val = 25.0
adhesive_modulus_val = 1000
adhesive_thickness_val = 0.005
...
def launch_rotation_tool(root):
    global image_filepath, start_y, end_y, threshold_value, start_x, end_x
    image_cropping(root)
    ...
    def run_rotation():
        RotationApp(root, input_folder, y_cut, x_cut, threshold, completion_callback=on_rotation_complete)
...
def couple_force_and_cycles(path, cycles):
    ...
    print(f"The displacement in the test was: {displacement_val:.2f}")

    cyc_force = []

    for cycle in cycles:
        if cycle == 1:
            tup = (cycles[0], machine_forces[0], machine_forces_min[0], 0, 0, displacement_max[0], displacement_min[0])
            ...
            for count, value in enumerate(machine_cycles):
                if (machine_cycles[count - 1] < cycle) and (cycle <= machine_cycles[count]):
                    ...
                    tup = (cycle, interpolation(data_force, cycle), interpolation(data_force_min, cycle),
                           0, 0, displacement_max[count], displacement_min[count])
                    cyc_force.append(tup)
                    break
    return cyc_force
    ...
    for i, image_file in enumerate(image_files):
        if image_file.endswith(('.jpg', '.jpeg', '.png')):
            try:
                ...
                # Re-initialize cut ranges inside loop
                ...
                white_pixel_middle = average_white_pixel_y(cut_image(image, x_cut_range, y_cut_range))
                ...
                if not lower_slope_points or not upper_slope_points:
                    print(f"WARNING: Empty point set at image {i}: lower={len(lower_slope_points)}, "
                          f"upper={len(upper_slope_points)}")
                ...
                # crack length a calculated using EFM
                a = ((((beta ** 8) * (3 * compliance * k - 4 * beta)) ** (1 / 3)) - 2 * beta ** 3) / (2 * beta ** 4)
                # G value based on simple beam theory and crack length calculated by EFM
                ...
                # percentage difference between LEFM and J-integral
                try:
                    if comparison_choice.get() == "G_CC":
                        perc_diff = (SERR_CC / j_integral_max - 1) * 100
                    elif comparison_choice.get() == "G_CC_mod":
                        perc_diff = (SERR_CC_mod / j_integral_max - 1) * 100
                    else:
                        perc_diff = (G_efm / j_integral_max - 1) * 100
                except Exception as e:
                    perc_diff = (SERR_CC / j_integral_max - 1) * 100

                perc_diff = perc_diff.tolist()
                header = header + f";Compliance;a-EFM;a-eq;G-EFM;%diff;displacement: {displacement_val:.2f}; " \
                                  f"x-cut: {x_cut_range}\n"
                data = data + f';{compliance:.4f};{a:.2f};{crack_with_jmax:.2f};{G_efm:.2f};{perc_diff:.2f}\n'
                ...
                messagebox.showinfo("Finished!", f"The Image Processing is completed, Last image was out of bounds!\n "
                                                 f"Evaluated images:{i}")
    messagebox.showinfo("Finished!", f"The Image Processing is completed!\n Evaluated images:{i}")
    # Close the popup
    popup.destroy()
...
def show_picture():
    ...
    plt.title("Image with fitted beam edges")
    plt.axis('off')
    plt.legend()
    plt.show()
...
def edit_parameters(root, back_col, label_col, button_col):
...
def browse_for_images(root):
    global image_filepath, cycles, frequency
    ...
    # Extract numbers before the 's' from image timestamps
    time_values = []
    for filename in image_files:
        match = re.search(r'_(\d+)s', filename)
        if match:
            time_values.append(int(match.group(1)))
    cycles = np.array(time_values)*frequency
    cycles[0] = 1
...
def browse_for_instron(root):
    ...
    global instron_filepath, outputpath
    print(file_path)
...
def browse_for_evaluation(root):
    ...
    if not file_path:
        ...
        messagebox.showinfo("Error!", "No file was selected or file path was not found!")
        return
    ...
    # Create and configure the crack graph app
    crack_graph_app = CrackGraphApp(root, callback=write_params)
    crack_graph_app.edit_parameters(
...
if __name__ == "__main__":
    ...
    app.set_entry(entry_key="entry_4", entry="-1, -1")
    app.set_entry(entry_key="entry_5", entry="-1, -1")
    ...
    app.change_button_command(button_key="button_12", new_command=lambda: reduce_data_by_crack_prop(
                       ...
                       SERR_col=elements["entry_1"].get(),
                       crack_col=elements["entry_6"].get()))
    app.change_button_command(button_key="button_13", new_command=lambda:edit_parameters(root=app,
                                                                                        button_col="#bababa",
                                                                                        back_col="#f0f0f0",
                                                                                        label_col= "#f0f0f0"))
